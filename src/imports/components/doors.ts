...
export class DoorComponent {
  ...
  constructor(
    ...
    this.openPosition = new Vector3(
      doorEntity.getComponent(Transform).position.x,
      doorEntity.getComponent(Transform).position.y,
      doorEntity.getComponent(Transform).position.z
    )
    ...
    if (!this.bCreated) {
      ...
      if (this.doorEnitities.length == 0) {
        for (const entityId in engine.getEntitiesWithComponent(DoorComponent)) {
          let entity: Entity = engine.getEntitiesWithComponent(DoorComponent)[
            entityId
          ] as Entity
        ...
        (e) => {
          ...
          switch (self.doorBehaviour) {
            ...
            case DoorTriggerBehaviour.CloseAndOpen:
              ...
              break
            case DoorTriggerBehaviour.Toggle:
              self.openEntity.getComponent(GLTFShape).visible =
                !self.openEntity.getComponent(GLTFShape).visible
              self.closeEntity.getComponent(GLTFShape).visible =
                !self.closeEntity.getComponent(GLTFShape).visible
              break
            default:
              break
