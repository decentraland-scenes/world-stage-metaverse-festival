...
export class FollowPathMoveComponent {
  ...
  constructor(
    ...
    globalSpeed: number,
    bActive: Boolean,
    finishBehavior: OnFinishPathBehavior,
    ...
    if (entityToMove.hasComponent(MoveComponent)) {
      ...
      entityToMove.addComponent(
        new MoveComponent(
          movementType,
          targetPoints[0].position,
          entityToMove,
          speed,
          bOrientAxisToMovement,
          false,
          callback
        )
      )
  ...
  moveToPoint(pointIndex: number, self: FollowPathMoveComponent = null) {
    if (
      !self &&
      this.entityToMove &&
      this.entityToMove.hasComponent(FollowPathMoveComponent)
    ) {
    ...
    if (self) {
      if (self.targetPoints.length > pointIndex) {
        ...
        self.moveComponent.movement.targetLocation =
          self.targetPoints[pointIndex].position
  ...
  moveToNextPoint(self: FollowPathMoveComponent = null) {
    ...
    if (self) {
      ...
      if (self.targetPoints.length > self.targetPointIndex) {
        ...
        self.moveComponent.movement.targetLocation =
          self.targetPoints[self.targetPointIndex].position

        if (self.targetPoints[self.targetPointIndex].speed > 0) {
          self.moveComponent.setSpeed(
            self.targetPoints[self.targetPointIndex].speed
          )
          self.moveComponent.activate()
        } else {
          self.moveComponent.setSpeed(self.globalSpeed)
