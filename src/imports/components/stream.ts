...
export class StreamVideo {
  ...
  constructor(entity: Entity, data: StreamVideoConstruct) {
    ...
    if (data.hoverPlay === undefined || data.hoverPlay === null)
      data.hoverPlay = 'Play'
    if (data.hoverPause === undefined || data.hoverPause === null)
      data.hoverPause = 'Pause'
    ...
    if (
      !entity.hasComponent(PlaneShape) &&
    ...
    if (data.imageUrl) {
      ...
      this.entityImage.addComponent(
        new Transform({ scale: new Vector3(1, -1, 1) })
      )
