...
class SimpleMove implements IMovement {
  ...
  update(dt: number) {
    if (this.bActive) {
      ...
      let distance = directionVectorBetweenTwoPoints(
        transform.position,
        this.targetLocation
      ).scale(dt * this.speed)
      ...
      if (
        Vector3.Distance(transform.position, this.targetLocation) <=
        distance.length()
      ) {
  ...
  setMovement(
    ...
    switch (movementType) {
      ...
      case MovementType.Aceleration:
        this.movement = new AcelerateMove(
          targetLocation,
          entityToMove,
          speed,
          bOrientAxisToMovement,
      ...
      case MovementType.Projectile:
        this.movement = new ProjectileMove(
          targetLocation,
          entityToMove,
          speed,
          bOrientAxisToMovement,
